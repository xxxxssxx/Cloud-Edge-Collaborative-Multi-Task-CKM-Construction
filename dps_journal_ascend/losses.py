  ...
  def step_fn(state, batch, accelerator):
    ...
    if train:
      with accelerator.accumulate(model):
        loss = loss_fn(model, batch) / accelerator.gradient_accumulation_steps
        accelerator.backward(loss)
        if accelerator.sync_gradients:
          norm = torch.nn.utils.clip_grad_norm_(parameters=model.parameters(), max_norm=1.0, norm_type=2.0)  # 全局梯度裁剪
