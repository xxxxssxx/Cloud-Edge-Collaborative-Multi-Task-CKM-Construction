...
def get_default_configs():
  ...
  training.batch_size = 16   # default 64
  training.num_iter_step = 100000  # 总epoch数 代码中的step就是epoch的意思 for iter step
  ...
  training.grad_accum_steps = 8  # 梯度累积
  ...
  # inpaint
  config.inpaint = inpaint = ml_collections.ConfigDict()
  inpaint.checkpoint_num = 3  # 用哪个检查点做inpainting
  inpaint.mask_type = 'downsample'  # block or downsample
  inpaint.block_size = 16  # block mask的大小
  inpaint.downsample_rate = 3  # downsample mask的下采样率

  # dp_sampling
  config.dp_sampling = dp_sampling = ml_collections.ConfigDict()
  # specify the task file: inpainting_config, super_resolution_config, clipping_recovery_config, ray_tracing_config
  dp_sampling.task_config = "./configs/dps_configs/ray_tracing_config.yaml" 
  dp_sampling.checkpoint_num = 50
