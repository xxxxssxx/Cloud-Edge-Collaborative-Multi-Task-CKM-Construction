...
class NCSNpp(nn.Module):
  ...
  def forward(self, x, time_cond):
    ...
    if self.embedding_type == 'fourier':
      ...
      temb = modules[m_idx](torch.log(used_sigmas))  # (batch_size=64, embedding_size=256)
      m_idx += 1
    ...
    if self.conditional:  # 这里的conditional是指noise condition
    ...
    for i_level in reversed(range(self.num_resolutions)):
      for i_block in range(self.num_res_blocks + 1):
        h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb)  # 这里出错
        m_idx += 1
      ...
      if i_level != 0:
        if self.resblock_type == 'ddpm':
          ...
          h = modules[m_idx](h, temb)  # 这里会对h上采样。从7下采样是3，但是从3上采样是6。这就是形状不匹配的来源。
          m_idx += 1
