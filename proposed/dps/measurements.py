...
class JointDegradationOperatorSTE(NonLinearOperator):
    ...
    def forward(self, data, **kwargs):
        ...
        # 对非建筑位置做 STE：
        #   forward: 和 clamp(gain) 一样
        #   backward: d out / d gain = 1
        gain_ste = gain + (gain_clamped - gain).detach()

        # 建筑像素强制为 -1（这里 out 对 gain 不再有依赖，梯度为 0）
        gain_out = torch.where(
            building_mask,
            torch.full_like(gain_ste, -1.0),
            gain_ste,
        )
